    ...
    <div class="container  mt-5 ">
        <div class="row h-100 justify-content-center align-items-center">
            <div class=" col-md-6 mt-5">
                <form action="/update/{{$friend->id}}" method="post">
                    @csrf
                    @method('PUT')
                    
                    
                    <label class="mt-3">FullName</label>
                    <input placeholder="Name" class="form-control @error('name') is_invalid @enderror"  value="{{$friend->name}}" type="text" name="name" >
                    @error('name')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                    <label class="mt-3">Email</label>
                    <input placeholder="Email" class="form-control @error('email') is_invalid @enderror"  value="{{$friend->email}}"  type="email" name="email" >
                    @error('email')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                    <label class="mt-3">Phone</label>
                    <input placeholder="Phone" class="form-control @error('phone') is_invalid @enderror"  value="{{$friend->phone}}" type="number" name="phone" >
                    @error('phone')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                    {{-- hadi dyal cities --}}
                        {{-- <div class="form-group col-md-4">
                            ...
                            <select id="inputState" class="form-control">
                                <option selected>{{$friend->city}}</option>
                                <option>...</option>
                            </select>
                        </div>  --}}
                            {{-- <label for="inputState">City</label>
